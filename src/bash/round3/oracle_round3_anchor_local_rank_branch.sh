...
mkdir -p ../logs
LOG_FILE="../logs/run_round3_anchor_local_rank_oracle_branch_$(date '+%Y_%m_%d').log"

log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] $1" | tee -a "$LOG_FILE"
}

log "Starting run_round3_anchor_local_rank_oracle_branch.sh script"
...
for prompt in "${PROMPTS[@]}"; do
    for subset in "${RUN_SUBSETS[@]}"; do
        suffix="oracle_round3_anchor_local_rank_${prompt}"
        ...
        # Intent: run the oracle branch evaluation script with the same params as anchor-local-rank.
        cmd=( python run_oracle_branch.py "${final_args[@]}" )
        printf -v cmd_str '%q ' "${cmd[@]}"
        log "Executing: $cmd_str"

        "${cmd[@]}" 2>&1 | tee -a "$LOG_FILE"
        if [ ${PIPESTATUS[0]} -ne 0 ]; then
            log "Error in subset=${subset} prompt=${prompt}"
            exit 1
        fi

        log "Completed subset=${subset} prompt=${prompt}"
        log "---"
    done
done
