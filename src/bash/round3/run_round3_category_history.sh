...
mkdir -p ../logs
LOG_FILE="../logs/run_round3_category_history_$(date '+%Y_%m_%d').log"

log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] $1" | tee -a "$LOG_FILE"
}

log "Starting run_round3_category_history.sh script"
...
COMMON_PARAMS=(
    ...
    # Intent: disable category-policy control to isolate history-prefix effect.
)
...
SUMMARIZED_CONTEXT_MODES=(
    "on"
    # "off"
)

for history_mode in "${HISTORY_PREFIX_MODES[@]}"; do
    for summarized_mode in "${SUMMARIZED_CONTEXT_MODES[@]}"; do
        for prompt in "${PROMPTS[@]}"; do
            for subset in "${RUN_SUBSETS[@]}"; do
                for anchor_mode in "${ANCHOR_LOCAL_RANK_MODES[@]}"; do
                    suffix="round3_category_history_${anchor_mode}_${prompt}_${history_mode}_summary_${summarized_mode}"
                    ...
                    # Intent: summary off uses raw rewrite evidence and doc-ID fallback history.
                    if [[ "$summarized_mode" == "off" ]]; then
                        final_args+=("--round3_summarized_context" "off")
                    else
                        final_args+=("--round3_summarized_context" "on")
                    fi
                    ...
                    log "Executing (history=${history_mode}, summary=${summarized_mode}): $cmd_str"

                    "${cmd[@]}" 2>&1 | tee -a "$LOG_FILE"
                    if [ ${PIPESTATUS[0]} -ne 0 ]; then
                        log "Error in subset=${subset} prompt=${prompt} anchor_mode=${anchor_mode} history=${history_mode} summary=${summarized_mode}"
                        exit 1
                    fi

                    log "Completed subset=${subset} prompt=${prompt} anchor_mode=${anchor_mode} history=${history_mode} summary=${summarized_mode}"
                    log "---"
                done
            done
        done
