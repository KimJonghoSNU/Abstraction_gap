...
mkdir -p ../logs
LOG_FILE="../logs/run_round3_ablation_$(date '+%Y_%m_%d').log"
...
log "Starting run_round3_ablation.sh script"
...
for subset in "${RUN_SUBSETS[@]}"; do
    for context in "${CONTEXTS[@]}"; do
        suffix="round3_${context}"
        NODE_EMB_PATH="${NODE_EMB_BASE}/${subset}/node_embs.diver.npy"
        REWRITE_CACHE_TAG="round3_${context}"
        ...
        final_args+=("--round3_rewrite_context" "$context")
        ...
        if [ ${PIPESTATUS[0]} -ne 0 ]; then
            log "Error in subset=${subset} context=${context}"
            exit 1
        fi

        log "Completed subset=${subset} context=${context}"
        log "---"
    done
done
