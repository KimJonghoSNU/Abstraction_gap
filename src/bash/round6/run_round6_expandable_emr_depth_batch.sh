...
mkdir -p ../logs
LOG_FILE="../logs/run_round6_expandable_emr_depth_batch_$(date '+%Y_%m_%d').log"

log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] $1" | tee -a "$LOG_FILE"
}

log "Starting run_round6_expandable_emr_depth_batch.sh script"
...
if [[ "${ROUND6_RESEAT_DEPTH_CONTROL}" == "1" ]]; then
    # Intent: enable the soft depth-batched reseat controller only in the copied launcher so the original EMR path stays untouched.
    COMMON_PARAMS+=(
...
log "round6_emr_memory=${ROUND6_EMR_MEMORY} round6_emr_topk=${ROUND6_EMR_TOPK} round6_emr_compression=${ROUND6_EMR_COMPRESSION} rewrite_prompt_name=${ROUND5_REWRITE_PROMPT_NAME} round6_emr_memory_max_tokens=${ROUND6_EMR_MEMORY_MAX_TOKENS} round6_reseat_depth_control=${ROUND6_RESEAT_DEPTH_CONTROL} round6_reseat_depth_batch_size=${ROUND6_RESEAT_DEPTH_BATCH_SIZE} round6_reseat_skip_seen_same_depth=${ROUND6_RESEAT_SKIP_SEEN_SAME_DEPTH}"
...
for selector_mode in "${SELECTOR_MODES[@]}"; do
    for query_source in "${QUERY_SOURCES[@]}"; do
        for subset in "${RUN_SUBSETS[@]}"; do
            ...
            suffix="round6_mrr_selector_accum_${selector_mode}_expandable_${ROUND6_EXPANDABLE_MODE}_emr_depthbatch"
