...
mkdir -p ../logs
LOG_FILE="../logs/run_round6_expandable_random_$(date '+%Y_%m_%d').log"

log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] $1" | tee -a "$LOG_FILE"
}

log "Starting run_round6_expandable_random.sh script"
    ...
    for query_source in "${QUERY_SOURCES[@]}"; do
        for seed in "${RANDOM_SEEDS[@]}"; do
            for subset in "${RUN_SUBSETS[@]}"; do
                ...
                # Intent: keep seed 0 suffix identical to the historical random control and separate only nonzero-seed runs.
                suffix="round6_mrr_selector_accum_${selector_mode}_expandable_${ROUND6_EXPANDABLE_MODE}_${ROUND6_EXPANDABLE_RESEAT_POLICY}"
                if [[ "$seed" != "0" ]]; then
