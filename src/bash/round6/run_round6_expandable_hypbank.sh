...
mkdir -p ../logs
LOG_FILE="../logs/run_round6_expandable_hypbank_$(date '+%Y_%m_%d').log"

log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] $1" | tee -a "$LOG_FILE"
}

log "Starting run_round6_expandable_hypbank.sh script"
...
ROUND6_HYPBANK="${ROUND6_HYPBANK:-1}"
ROUND6_CONTEXT_PACKING="${ROUND6_CONTEXT_PACKING:-0}"
...
for selector_mode in "${SELECTOR_MODES[@]}"; do
    for query_source in "${QUERY_SOURCES[@]}"; do
        for subset in "${RUN_SUBSETS[@]}"; do
            ...
            suffix="round6_mrr_selector_accum_${selector_mode}_expandable_${ROUND6_EXPANDABLE_MODE}_hypbank"
            ...
            if [[ "$ROUND6_CONTEXT_PACKING" == "1" ]]; then
                # Intent: keep reseat-aware context packing opt-in so baseline hypbank runs remain directly comparable.
