...
mkdir -p ../logs
LOG_FILE="../logs/run_round6_expandable_emr_direct_child_depth_$(date '+%Y_%m_%d').log"

log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] $1" | tee -a "$LOG_FILE"
}

log "Starting run_round6_expandable_emr_direct_child_depth.sh script"
...
ROUND6_EXPANDABLE_ENDED_SCOPE="${ROUND6_EXPANDABLE_ENDED_SCOPE:-goexplore_direct_child}"
...
ROUND6_RESEAT_DEPTH_CONTROL="${ROUND6_RESEAT_DEPTH_CONTROL:-1}"
ROUND6_RESEAT_DEPTH_BATCH_SIZE="${ROUND6_RESEAT_DEPTH_BATCH_SIZE:-$MAX_BEAM_SIZE}"
ROUND6_RESEAT_SKIP_SEEN_SAME_DEPTH="${ROUND6_RESEAT_SKIP_SEEN_SAME_DEPTH:-1}"
if [[ "${ROUND6_EMR_MEMORY}" == "1" && "${ROUND5_REWRITE_PROMPT_NAME}" == "agent_executor_v1_icl2" ]]; then
    # Intent: EMR runs should default to the memory-aware rewrite prompt instead of silently using the non-memory prompt.
...
if [[ "${ROUND6_EMR_MEMORY}" == "1" ]]; then
    # Intent: this launcher is explicitly the EMR variant, so always pass round6 EMR flags when enabled.
    COMMON_PARAMS+=(
        --round6_emr_memory
...
if [[ "${ROUND6_RESEAT_DEPTH_CONTROL}" == "1" ]]; then
    # Intent: enabling depth control is what switches run_round6.py into reseat_depth_batch_v1 behavior.
    COMMON_PARAMS+=(
        --round6_reseat_depth_control
        --round6_reseat_depth_batch_size "$ROUND6_RESEAT_DEPTH_BATCH_SIZE"
    )
    if [[ "${ROUND6_RESEAT_SKIP_SEEN_SAME_DEPTH}" == "1" ]]; then
        COMMON_PARAMS+=(--round6_reseat_skip_seen_same_depth)
    fi
fi
...
log "round6_expandable_ended_scope=${ROUND6_EXPANDABLE_ENDED_SCOPE} round6_emr_memory=${ROUND6_EMR_MEMORY} round6_emr_topk=${ROUND6_EMR_TOPK} round6_emr_compression=${ROUND6_EMR_COMPRESSION} rewrite_prompt_name=${ROUND5_REWRITE_PROMPT_NAME} round6_reseat_depth_control=${ROUND6_RESEAT_DEPTH_CONTROL} round6_reseat_depth_batch_size=${ROUND6_RESEAT_DEPTH_BATCH_SIZE} round6_reseat_skip_seen_same_depth=${ROUND6_RESEAT_SKIP_SEEN_SAME_DEPTH}"
...
for selector_mode in "${SELECTOR_MODES[@]}"; do
    for query_source in "${QUERY_SOURCES[@]}"; do
        for subset in "${RUN_SUBSETS[@]}"; do
            ...
            # Intent: encode that this launcher combines direct-child reseat, depth batching, and EMR in one fixed experiment family.
            suffix="round6_mrr_selector_accum_${selector_mode}_expandable_${ROUND6_EXPANDABLE_MODE}_emr_directchild_depthbatch"
