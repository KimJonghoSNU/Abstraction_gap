...
mkdir -p ../logs
LOG_FILE="../logs/run_round6_expandable_frontiercum_beampack_$(date '+%Y_%m_%d').log"

log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] $1" | tee -a "$LOG_FILE"
}

log "Starting run_round6_expandable_frontiercum_beampack.sh script"
...
if [[ "${ROUND6_EMR_MEMORY}" == "1" && "${ROUND5_REWRITE_PROMPT_NAME}" == "agent_executor_v1_icl2" ]]; then
    # Intent: keep beampack defaults stable while auto-switching only EMR runs to the memory-aware prompt.
...
if [[ "${ROUND6_EMR_MEMORY}" == "1" ]]; then
    # Intent: expose EMR as an optional overlay on top of beam-per-evidence without changing the default beampack path.
    COMMON_PARAMS+=(
        --round6_emr_memory
...
for selector_mode in "${SELECTOR_MODES[@]}"; do
    for query_source in "${QUERY_SOURCES[@]}"; do
        for subset in "${RUN_SUBSETS[@]}"; do
            ...
            suffix="round6_mrr_selector_accum_${selector_mode}_expandable_${ROUND6_EXPANDABLE_MODE}_frontiercum_beampack"
            if [[ "${ROUND6_EMR_MEMORY}" == "1" ]]; then
                suffix="${suffix}_emr"
            fi
            ...
            cmd=( python run_round6_beampack.py "${final_args[@]}" )
