...
mkdir -p ../logs
LOG_FILE="../logs/run_round6_expandable_context_pack_$(date '+%Y_%m_%d').log"

log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] $1" | tee -a "$LOG_FILE"
}

log "Starting run_round6_expandable_context_pack.sh script"
...
ROUND6_HYPBANK="${ROUND6_HYPBANK:-0}"
ROUND6_CONTEXT_PACKING="${ROUND6_CONTEXT_PACKING:-1}"
ROUND6_HYPBANK_CONTEXT_PACKING="${ROUND6_HYPBANK_CONTEXT_PACKING:-0}"
if [[ "$ROUND6_HYPBANK_CONTEXT_PACKING" == "1" ]]; then
    ROUND6_CONTEXT_PACKING="1"
fi
...
for selector_mode in "${SELECTOR_MODES[@]}"; do
    for query_source in "${QUERY_SOURCES[@]}"; do
        for subset in "${RUN_SUBSETS[@]}"; do
            ...
            suffix="round6_mrr_selector_accum_${selector_mode}_expandable_${ROUND6_EXPANDABLE_MODE}_frontiercum"
            if [[ "${ROUND6_EXPANDABLE_RESEAT_POLICY}" != "score" ]]; then
                suffix="${suffix}_${ROUND6_EXPANDABLE_RESEAT_POLICY}"
            fi
            if [[ "$ROUND6_CONTEXT_PACKING" == "1" ]]; then
                suffix="${suffix}_ctxpack"
            fi
            ...
            if [[ "$ROUND6_HYPBANK" == "1" ]]; then
                # Intent: keep the copied launcher opt-in for reseat-time hypothesis pruning while preserving compatibility when disabled.
                final_args+=("--round6_hypbank")
            fi
            if [[ "$ROUND6_CONTEXT_PACKING" == "1" ]]; then
                # Intent: context-pack-only runs should isolate the prompt-side old/new evidence split without turning on hypbank retention logic.
                final_args+=("--round6_context_packing")
            fi

            cmd=( python run_round6_hypbank.py "${final_args[@]}" )
