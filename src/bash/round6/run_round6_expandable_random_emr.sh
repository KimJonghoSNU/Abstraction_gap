...
mkdir -p ../logs
LOG_FILE="../logs/run_round6_expandable_random_emr_$(date '+%Y_%m_%d').log"

log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] $1" | tee -a "$LOG_FILE"
}

log "Starting run_round6_expandable_random_emr.sh script"
...
ROUND6_EXPANDABLE_RESEAT_POLICY="${ROUND6_EXPANDABLE_RESEAT_POLICY:-random}"
ROUND5_REWRITE_PROMPT_NAME="${ROUND5_REWRITE_PROMPT_NAME:-agent_executor_v1_icl2}"
ROUND6_RANDOM_SEEDS="${ROUND6_RANDOM_SEEDS:-1 2}"
...
if [[ "${ROUND6_EMR_MEMORY}" == "1" && "${ROUND5_REWRITE_PROMPT_NAME}" == "agent_executor_v1_icl2" ]]; then
    # Intent: keep the default random-reseat launcher stable while auto-switching prompt text only for EMR runs.
...
if [[ "$ROUND5_DISABLE_CALIBRATION" == "1" ]]; then
    # Intent: default to retriever-score-only branch logic in the expandable random reseat ablation.
...
log "round6_emr_memory=${ROUND6_EMR_MEMORY} round6_emr_topk=${ROUND6_EMR_TOPK} round6_emr_compression=${ROUND6_EMR_COMPRESSION} rewrite_prompt_name=${ROUND5_REWRITE_PROMPT_NAME} round6_emr_memory_max_tokens=${ROUND6_EMR_MEMORY_MAX_TOKENS} round6_random_seeds=${ROUND6_RANDOM_SEEDS}"

read -r -a SELECTOR_MODES <<< "$ROUND5_SELECTOR_MODES"
read -r -a RANDOM_SEEDS <<< "$ROUND6_RANDOM_SEEDS"
    ...
    for query_source in "${QUERY_SOURCES[@]}"; do
        for seed in "${RANDOM_SEEDS[@]}"; do
            for subset in "${RUN_SUBSETS[@]}"; do
                ...
                # Intent: keep seed 0 naming compatible with the existing random line and separate only EMR/nonzero-seed variants.
                suffix="round6_mrr_selector_accum_${selector_mode}_expandable_${ROUND6_EXPANDABLE_MODE}_emr_${ROUND6_EXPANDABLE_RESEAT_POLICY}"
                if [[ "$seed" != "0" ]]; then
                    suffix="${suffix}_seed${seed}"
                fi
                ...
                if [[ -n "$ROUND5_REWRITE_PROMPT_NAME" ]]; then
                    # Intent: expose the rewrite prompt override while keeping the ablation focused on random reseat plus EMR memory.
                    final_args+=("--rewrite_prompt_name" "$ROUND5_REWRITE_PROMPT_NAME")
                fi

                cmd=( python run_round6.py "${final_args[@]}" )
                printf -v cmd_str '%q ' "${cmd[@]}"
                log "Executing seed=${seed}: $cmd_str"

                "${cmd[@]}" 2>&1 | tee -a "$LOG_FILE"
                if [ ${PIPESTATUS[0]} -ne 0 ]; then
                    log "Error in selector_mode=${selector_mode} subset=${subset} query_source=${query_source} seed=${seed}"
                    exit 1
                fi

                log "Completed selector_mode=${selector_mode} subset=${subset} query_source=${query_source} seed=${seed}"
                log "---"
            done
        done
    done
