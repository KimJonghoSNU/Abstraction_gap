...
mkdir -p ../logs
LOG_FILE="../logs/run_round4_category_fusion_$(date '+%Y_%m_%d').log"

log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] $1" | tee -a "$LOG_FILE"
}

log "Starting run_round4_category_fusion.sh script"
...
for fusion_mode in "${CATEGORY_FUSIONS[@]}"; do
    for prompt in "${PROMPTS[@]}"; do
        for subset in "${RUN_SUBSETS[@]}"; do
            for anchor_mode in "${ANCHOR_LOCAL_RANK_MODES[@]}"; do
                suffix="round4_category_fusion_${fusion_mode}_${anchor_mode}_${prompt}"
                ...
                final_args+=("--category_fusion" "$fusion_mode")
                ...
                log "Executing (fusion=${fusion_mode}): $cmd_str"

                "${cmd[@]}" 2>&1 | tee -a "$LOG_FILE"
                if [ ${PIPESTATUS[0]} -ne 0 ]; then
                    log "Error in subset=${subset} prompt=${prompt} anchor_mode=${anchor_mode} fusion=${fusion_mode}"
                    exit 1
                fi

                log "Completed subset=${subset} prompt=${prompt} anchor_mode=${anchor_mode} fusion=${fusion_mode}"
                log "---"
            done
        done
    done
...
log "Finished run_round4_category_fusion.sh script"
