...
mkdir -p ../logs
LOG_FILE="../logs/run_round4_gold_keys_$(date '+%Y_%m_%d').log"

log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] $1" | tee -a "$LOG_FILE"
}

log "Starting run_round4_gold_keys.sh script"
...
for prompt in "${PROMPTS[@]}"; do
    for subset in "${RUN_SUBSETS[@]}"; do
        for anchor_mode in "${ANCHOR_LOCAL_RANK_MODES[@]}"; do
            suffix="round4_gold_keys_anchor_local_rank_${anchor_mode}_${prompt}"
            ...
            # Intent: run the gold-key variant where rewrite keys are constrained by per-sample selected corpus categories.
            cmd=( python run_round4_gold_keys.py "${final_args[@]}" )
