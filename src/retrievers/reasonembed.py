...
class ReasonEmbedEmbeddingModel:
    """
    HuggingFace wrapper for reason-embed style dense retrieval.
    ...
    def _embedding_dim(self) -> int:
        ...
        if hidden_size is None:
            raise ValueError("ReasonEmbed model config is missing hidden_size")
    ...
    def encode_docs(
        ...
        *,
        ...
        docs_list = [doc if isinstance(doc, str) else "" for doc in docs]
        valid_pairs = [(idx, doc) for idx, doc in enumerate(docs_list) if doc.strip()]

        if not keep_empty_rows:
            docs_clean = [doc for _, doc in valid_pairs]
            return self.encode(docs_clean, max_length=max_length, batch_size=batch_size)

        emb_dim = self._embedding_dim()
        # Intent: node-catalog export must preserve one embedding row per registry entry, including blank roots.
        output = np.zeros((len(docs_list), emb_dim), dtype=np.float32)
        if not valid_pairs:
            return output

        valid_indices = [idx for idx, _ in valid_pairs]
        valid_docs = [doc for _, doc in valid_pairs]
        valid_embs = self.encode(valid_docs, max_length=max_length, batch_size=batch_size)
        output[np.asarray(valid_indices, dtype=np.int64)] = valid_embs
        return output
