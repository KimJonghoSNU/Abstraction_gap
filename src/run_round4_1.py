...
ROUND4_1_USE_PREPLANNER = True
ROUND4_1_PREPLANNER_PROMPT_NAME = "round4_preplanner_v1"
ROUND4_1_PREPLANNER_PROMPT_NAME_DOC_ID = "round4_preplanner_docid_v1"
ROUND4_1_PREPLANNER_MAX_KEYWORDS = 10
ROUND4_1_PREPLANNER_MAX_CATEGORIES = 5
ROUND4_1_PREPLANNER_REFERENCE_MODES = {"website_title", "doc_id"}
...
def _clean_website_title(raw_title: str) -> str:
    title = str(raw_title or "").strip()
    if not title:
        return ""
    # Intent: pre-planner title pool should remove file-extension and numeric noise before LLM reads it.
    title = re.sub(r"\.txt\b", " ", title, flags=re.IGNORECASE)
    title = re.sub(r"\.html?\b", " ", title, flags=re.IGNORECASE)
    title = re.sub(r"\d+", " ", title)
    title = re.sub(r"\s+", " ", title).strip()
    return title


def _build_website_titles_hint(base_dir: str, dataset: str, subset: str) -> str:
    jsonl_path = os.path.join(
        base_dir,
        "trees",
        str(dataset),
        str(subset),
        "document_categories_category_assign_v2.jsonl",
    )
    ...
    with open(jsonl_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                continue
            raw_title = str(row.get("website_title", "")).strip()
            if not raw_title:
                doc_id = str(row.get("doc_id", "")).strip()
                if doc_id:
                    raw_title = os.path.splitext(os.path.basename(doc_id))[0]
            clean_title = _clean_website_title(raw_title)
            if not clean_title:
                continue
...
def _build_doc_ids_hint(base_dir: str, dataset: str, subset: str) -> str:
    jsonl_path = os.path.join(
        base_dir,
        "trees",
        str(dataset),
...
def _build_preplanner_reference_pool_hint(
    base_dir: str,
    dataset: str,
    subset: str,
    reference_mode: str,
) -> str:
    mode = str(reference_mode or "").strip().lower()
    if mode == "doc_id":
        return _build_doc_ids_hint(base_dir, dataset, subset)
    return _build_website_titles_hint(base_dir, dataset, subset)
...
def _to_clean_str_list(value: object, max_items: int) -> List[str]:
    if max_items <= 0:
        return []
    items: List[str] = []
...
def _parse_preplanner_output(
    text: str,
    max_keywords: int,
    max_categories: int,
) -> Dict[str, object]:
    obj = _parse_json_object(text) or {}
    raw_keywords = obj.get("Branch_Keywords") or obj.get("branch_keywords") or obj.get("keywords") or []
    raw_categories = obj.get("Selected_Categories") or obj.get("selected_categories") or obj.get("categories") or []
    keywords = _to_clean_str_list(raw_keywords, max_keywords)
    categories = _to_clean_str_list(raw_categories, max_categories)
    return {
...
def _build_preplanner_hint(parsed: Dict[str, object], reference_mode: str = "website_title") -> str:
    lines: List[str] = []
    categories = [str(x).strip() for x in (parsed.get("selected_categories") or []) if str(x).strip()]
    keywords = [str(x).strip() for x in (parsed.get("branch_keywords") or []) if str(x).strip()]
    keyword_source_label = "website titles"
    if str(reference_mode or "").strip().lower() == "doc_id":
        keyword_source_label = "document IDs"
    if categories:
        lines.append("Selected categories from pre-planner:")
        for c in categories:
            lines.append(f"- {c}")
    if keywords:
        lines.append(f"Relevant keywords from {keyword_source_label}:")
        for k in keywords:
            lines.append(f"- {k}")
    return "\n".join(lines).strip()
...
def _format_action_prompt(
    ...
    try:
        prompt = template.format(
            ...
            preplanner_hint=preplanner_hint,
        )
        ...
        prompt = (
            ...
            .replace("{preplanner_hint}", preplanner_hint)
        )
        return prepend_history_to_prompt(prompt, retrieval_history)


def _format_preplanner_prompt(
    template: str,
    original_query: str,
    previous_rewrite: str,
    leaf_descs: List[str],
    ...
    try:
        return template.format(
            ...
            leaf_descs=leaf_blob,
            branch_descs=branch_blob,
            corpus_categories=(corpus_categories or ""),
            website_titles=(reference_pool or ""),
            doc_ids=(reference_pool or ""),
        ...
        return (
            ...
            .replace("{leaf_descs}", leaf_blob)
            .replace("{branch_descs}", branch_blob)
            .replace("{corpus_categories}", corpus_categories or "")
            .replace("{website_titles}", reference_pool or "")
            .replace("{doc_ids}", reference_pool or "")
            .replace("{max_keywords}", str(int(max_keywords)))
            .replace("{max_categories}", str(int(max_categories)))
        )
...
if anchor_mode not in {"none", "v2"}:
    raise ValueError(
        f'Unsupported --round3_anchor_local_rank="{hp.ROUND3_ANCHOR_LOCAL_RANK}" in run_round4.py. '
        'Allowed: none|v2'
    )
if bool(hp.ROUND3_REWRITE_USE_HISTORY):
    print("Ignoring --round3_rewrite_use_history in run_round4.py (fixed to False).")
if hp.ROUND3_ROUTER_PROMPT_NAME:
    print("Ignoring --round3_router_prompt_name in run_round4.py (router is disabled).")
if str(hp.ROUND3_SUMMARIZED_CONTEXT or "off").lower() != "off":
    print("Ignoring --round3_summarized_context in run_round4.py (fixed to off).")
...
round4_preplanner_reference_mode = str(getattr(hp, "ROUND4_PREPLANNER_REFERENCE_MODE", "website_title") or "website_title").lower()
if round4_preplanner_reference_mode not in ROUND4_1_PREPLANNER_REFERENCE_MODES:
    raise ValueError(
        f'Unsupported --round4_preplanner_reference_mode="{round4_preplanner_reference_mode}". '
        "Allowed: website_title|doc_id"
    )
hp.add_param("round4_preplanner_reference_mode", round4_preplanner_reference_mode)
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
corpus_categories_hint = _build_corpus_categories_hint(BASE_DIR, hp.DATASET, hp.SUBSET)
# Intent: pre-planner can switch between title-based and doc_id-based reference pools for A/B comparison.
preplanner_reference_pool_hint = _build_preplanner_reference_pool_hint(
    BASE_DIR,
    hp.DATASET,
    hp.SUBSET,
...
exp_dir_name = str(hp)
# Intent: run_round4_1 isolates pre-planner experiments under round4_1 for safe comparison against round4 baselines.
RESULTS_DIR = f"{BASE_DIR}/results/{hp.DATASET}/{hp.SUBSET}/round4_1/{exp_dir_name}/"
...
logger = setup_logger("lattice_runner_round4_1", log_path, logging.INFO)
...
preplanner_template = None
if ROUND4_1_USE_PREPLANNER:
    preplanner_prompt_name = ROUND4_1_PREPLANNER_PROMPT_NAME
    if round4_preplanner_reference_mode == "doc_id":
        preplanner_prompt_name = ROUND4_1_PREPLANNER_PROMPT_NAME_DOC_ID
    if preplanner_prompt_name not in REWRITE_PROMPT_TEMPLATES:
        raise ValueError(
            f'Unknown pre-planner template "{preplanner_prompt_name}". '
            f"Available: {sorted(REWRITE_PROMPT_TEMPLATES.keys())}"
        )
    preplanner_template = REWRITE_PROMPT_TEMPLATES[preplanner_prompt_name]
rewrite_map: Dict[str, str] = {}
...
for iter_idx in range(hp.NUM_ITERS):
    ...
    if rewrite_candidates and ROUND4_1_USE_PREPLANNER and preplanner_template:
        preplanner_prompts: List[str] = []
        preplanner_meta: List[Dict] = []
        for meta in rewrite_candidates:
            sample = meta["sample"]
            leaf_descs = meta["leaf_descs"]
            branch_descs = meta["branch_descs"] if hp.ROUND3_REWRITE_CONTEXT == "leaf_branch" else []
            pre_prompt = _format_preplanner_prompt(
                preplanner_template,
                sample.original_query,
                sample.last_rewrite,
                leaf_descs,
                branch_descs,
                corpus_categories_hint,
                preplanner_reference_pool_hint,
                ROUND4_1_PREPLANNER_MAX_KEYWORDS,
                ROUND4_1_PREPLANNER_MAX_CATEGORIES,
            )
            preplanner_prompts.append(pre_prompt)
            preplanner_meta.append(meta)
        logger.info("Iter %d: starting pre-planner batch (%d prompts)", iter_idx, len(preplanner_prompts))
        pre_loop = asyncio.new_event_loop()
        asyncio.set_event_loop(pre_loop)
        try:
            preplanner_outputs = pre_loop.run_until_complete(
                llm_api.run_batch(preplanner_prompts, **llm_api_kwargs)
            )
        finally:
            pre_loop.close()
            asyncio.set_event_loop(None)
        for meta, out in zip(preplanner_meta, preplanner_outputs):
            parsed = _parse_preplanner_output(
                out,
                ROUND4_1_PREPLANNER_MAX_KEYWORDS,
                ROUND4_1_PREPLANNER_MAX_CATEGORIES,
            )
            meta["preplanner"] = parsed
            meta["preplanner_hint"] = _build_preplanner_hint(parsed, round4_preplanner_reference_mode)
    else:
        for meta in rewrite_candidates:
            meta["preplanner"] = {}
            meta["preplanner_hint"] = ""
    ...
    if rewrite_candidates:
        ...
        for meta in rewrite_candidates:
            ...
            preplanner_hint = str(meta.get("preplanner_hint", "") or "")
            ...
            if (not hp.REWRITE_FORCE_REFRESH) and (cache_key in rewrite_map):
                ...
                sample.rewrite_history.append({
                    ...
                    "preplanner": meta.get("preplanner", {}),
                    "preplanner_hint": preplanner_hint,
        ...
        if rewrite_prompts:
            ...
            for meta, out in zip(rewrite_meta, rewrite_outputs):
                ...
                sample.rewrite_history.append({
                    ...
                    "preplanner": meta.get("preplanner", {}),
                    "preplanner_hint": meta.get("preplanner_hint", ""),
                    "action": sample.last_action,
                    "actions": sample.last_actions,
                    "possible_docs": sample.last_possible_docs,
                    "rewrite": rewrite,
