REWRITE_PROMPT_TEMPLATES = {
    ...
    "leaf_stop_judge_v1": (
        "You are deciding whether another retrieval rewrite is still necessary.\n\n"
        "Subset Relevance Definition:\n"
        "{relevance_definition}\n\n"
        "Task:\n"
        "- Return exactly `stop` if you are certain no further improvement is possible.\n"
        "- Return exactly `continue` otherwise.\n"
        "- If documents are semantically similar but do not help answer the question, do not stop.\n\n"
        "- The relevant documents may not answer the question directly. They provide useful background information that would be cited in a correct answer.\n"
        "Original Query:\n{original_query}\n\n"
        "Previous Rewritten Query:\n{previous_rewrite}\n\n"
        "Context Summaries:\n{gate_descs}\n"
    ),
    "agent_executor_v1_icl2_context_pack": (
        "You are rewriting a search query for reasoning-intensive retrieval.\n\n"
        "Key idea:\n"
        "- Assume you already have the correct answer to the user query.\n"
        "- The documents we want are the ones that would be used as core evidence or justification for that answer.\n"
        ...
        "Task:\n"
        "- First, write a 1-2 sentence Plan that states the user's core intent and what kind of evidence would justify the answer.\n"
        "- Use both evidence blocks only as hints (not ground truth).\n"
        "- Carryover Evidence reflects previously reachable directions that may still be useful.\n"
        "- New Branch Evidence reflects documents from newly added reseat branches and should be used to explore a different plausible evidence route when appropriate.\n"
        "- Produce 2-5 distinct Possible_Answer_Docs that could serve as evidence for the assumed correct answer.\n"
        "\t- For example, propose multiple plausible theories/mechanisms, concrete entities/phenomena, or examples/analogies. Propose other possible useful documents.\n"
        "**Output Format:**\n"
        "Output a single JSON object. The values will be concatenated to form the next query.\n"
