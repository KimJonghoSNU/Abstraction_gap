...
def _score_candidate_branches_mrr(
    *,
    ...
    for candidate in candidate_child_paths:
        ...
        for rank_idx, hit in enumerate(local_hits):
            ...
            if (cand_t == leaf_path) or (cand_t in ancestor_paths):
                ...
                best_rr = float(1.0 / float(best_rank))
                best_leaf_score = float(hit.score)
        ...
        rows.append(
            {
                ...
                "best_leaf_score": float(best_leaf_score) if np.isfinite(best_leaf_score) else None,
    ...
    rows.sort(
        key=lambda r: (
            -float(r.get("mrr", 0.0)),
            -float(r.get("best_leaf_score", float("-inf")) or float("-inf")),
            len(r.get("path", [])),
            tuple(r.get("path", [])),
        )
    )
...
def _select_next_branches(
    *,
    scored_rows: Sequence[Dict[str, Any]],
    beam_size: int,
    previous_selected: Sequence[Tuple[int, ...]],
    root_branch_children: Sequence[Tuple[int, ...]],
) -> Tuple[List[Tuple[int, ...]], str]:
    if scored_rows:
        selected = [tuple(row.get("path", [])) for row in scored_rows[: max(1, int(beam_size))]]
        selected = [p for p in selected if p]
        if selected:
            return selected, "mrr_topb"
    if previous_selected:
        # Intent: keep traversal stable when no candidate can be scored in current step.
        return [tuple(p) for p in previous_selected[: max(1, int(beam_size))]], "carry_previous"
    if root_branch_children:
        return [tuple(p) for p in root_branch_children[: max(1, int(beam_size))]], "root_fallback"
    return [], "empty"
...
logger.info(
    "Round5 config | mode=%s | rewrite_prompt=%s | category_generator_prompt=%s | category_k=%d",
    round5_mode,
...
for iter_idx in range(hp.NUM_ITERS):
    ...
    for sample_idx, sample in enumerate(all_eval_samples):
        selected_before_by_idx[sample_idx] = _selected_branch_paths_from_sample(sample)
