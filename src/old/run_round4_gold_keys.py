...
def _clean_str_list(value: object, max_items: int = 16) -> List[str]:
    out: List[str] = []
...
def _load_doc_category_keywords_map(base_dir: str, dataset: str, subset: str) -> Dict[str, List[str]]:
    path = os.path.join(
        base_dir,
        "trees",
        str(dataset),
    ...
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if not doc_id:
                continue
            labels: List[str] = []
            labels.extend(_clean_str_list(row.get("category_level_2_keywords", []), max_items=12))
            labels.extend(_clean_str_list(row.get("category_level_1_keywords", []), max_items=8))
            for key in ("category_level_2", "category_level_1", "category"):
            ...
            seen: set[str] = set()
            for label in labels:
                if not _is_valid_category_label(label):
                    continue
                ...
                if len(clean) >= 12:
                    break
            if clean:
                mapping[doc_id] = clean
    return mapping


def _build_default_selected_categories(base_dir: str, dataset: str, subset: str, max_items: int = 3) -> List[str]:
    registry_path = os.path.join(
        base_dir,
        "trees",
        str(dataset),
    ...
    for row in payload.get("categories", []):
        if not isinstance(row, dict):
            continue
        label = str(row.get("level2", row.get("label", ""))).strip()
        if not _is_valid_category_label(label):
            continue
        try:
            ...
            cnt = 0
        scores[label] = scores.get(label, 0) + cnt
    if not scores:
        return ["Theory", "Method", "Evidence"]
    ordered = sorted(scores.items(), key=lambda x: (-int(x[1]), str(x[0])))
    return [label for label, _ in ordered[: max(1, int(max_items))]]


def _select_gold_categories_for_docs(
    gold_doc_ids: Sequence[str],
    doc_category_map: Dict[str, List[str]],
    max_keys: int,
    ...
    for doc_id in gold_doc_ids:
        labels = doc_category_map.get(str(doc_id), [])
        if not labels:
            continue
        hit_count += 1
        for rank, label in enumerate(labels):
            text = str(label).strip()
            if not _is_valid_category_label(text):
                continue
            if text not in order_by_label:
                order_by_label[text] = len(order_by_label)
            # Intent: earlier category keywords from each gold document get higher voting weight.
            score_by_label[text] = score_by_label.get(text, 0.0) + (1.0 / float(rank + 1))
    if hit_count == 0 or (not score_by_label):
        return [str(x).strip() for x in fallback if str(x).strip()][: max(1, int(max_keys))]
    ordered = sorted(
        score_by_label.items(),
        key=lambda x: (-float(x[1]), int(order_by_label.get(x[0], 10_000)), str(x[0])),
    )
    return [label for label, _ in ordered[: max(1, int(max_keys))]]


def _append_selected_corpus_category_keys_prompt(prompt: str, selected_categories: Sequence[str]) -> str:
    keys = [str(x).strip() for x in selected_categories if str(x).strip()]
    if not keys:
        return prompt
    bullets = "\n".join([f"- {key}" for key in keys])
    example_keys = keys[: min(4, len(keys))]
    example_obj = {"Possible_Answer_Docs": {key: "..." for key in example_keys}}
    example_json = json.dumps(example_obj, ensure_ascii=False, indent=2)
    suffix = (
        "\n\nGold-selected corpus categories for this sample (allowed keys):\n"
        f"{bullets}\n"
        "Output constraints:\n"
        "- In Possible_Answer_Docs, keys MUST be chosen only from the list above.\n"
        "- Do not use generic placeholders like Category A, Category B.\n"
        "- Keep each value concrete and retrieval-friendly.\n"
        "Example with this sample's selected keys:\n"
        f"{example_json}\n"
    )
    return (str(prompt or "").strip() + suffix).strip()
...
if not hp.REWRITE_PROMPT_NAME and not hp.REWRITE_PROMPT_PATH and not hp.REWRITE_CACHE_PATH:
    # Intent: gold-key analysis uses v5-style category-aware rewrite by default.
    hp.add_param("rewrite_prompt_name", "round3_agent_executor_v5")
# Intent: round4-gold-keys scope supports only none/v2 anchor modes to keep code path auditable.
anchor_mode = str(hp.ROUND3_ANCHOR_LOCAL_RANK or "none").lower()
if anchor_mode not in {"none", "v2"}:
    raise ValueError(
        f'Unsupported --round3_anchor_local_rank="{hp.ROUND3_ANCHOR_LOCAL_RANK}" in run_round4_gold_keys.py. '
        'Allowed: none|v2'
    )
if bool(hp.ROUND3_REWRITE_USE_HISTORY):
    print("Ignoring --round3_rewrite_use_history in run_round4_gold_keys.py (fixed to False).")
if hp.ROUND3_ROUTER_PROMPT_NAME:
    print("Ignoring --round3_router_prompt_name in run_round4_gold_keys.py (router is disabled).")
if str(hp.ROUND3_SUMMARIZED_CONTEXT or "off").lower() != "off":
    print("Ignoring --round3_summarized_context in run_round4_gold_keys.py (fixed to off).")
...
if round4_iter0_prompt_name:
    print("Ignoring --round4_iter0_prompt_name in run_round4_gold_keys.py (iter0 uses same prompt as later iters).")
round4_iter0_prompt_name = ""
hp.add_param("round4_iter0_prompt_name", None)
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
corpus_categories_hint = _build_corpus_categories_hint(BASE_DIR, hp.DATASET, hp.SUBSET)
exp_dir_name = str(hp)
# Intent: isolate gold-key analysis artifacts under a dedicated directory for clean comparison with round4 baselines.
RESULTS_DIR = f"{BASE_DIR}/results/{hp.DATASET}/{hp.SUBSET}/round4_gold_keys/{exp_dir_name}/"
...
logger = setup_logger("lattice_runner_round4_gold_keys", log_path, logging.INFO)
...
doc_category_keywords_map = _load_doc_category_keywords_map(BASE_DIR, hp.DATASET, hp.SUBSET)
default_gold_selected_categories = _build_default_selected_categories(
    BASE_DIR,
    hp.DATASET,
    hp.SUBSET,
    max_items=3,
...
logger.info(
    "Gold category mapping loaded: %d docs with category keywords (fallback defaults: %s)",
    len(doc_category_keywords_map),
    default_gold_selected_categories,
)
...
gold_selected_categories_by_sample: List[List[str]] = []
for i in range(num_samples):
    gold_ids = [str(doc_id) for doc_id in list(examples_df.iloc[i]["gold_ids"])]
    gold_paths = [doc_id_to_path[doc_id] for doc_id in gold_ids if doc_id in doc_id_to_path]
    if len(gold_paths) < len(gold_ids):
        logger.warning(f"Some gold IDs for example {i} not found in document paths.")
    selected_gold_categories = _select_gold_categories_for_docs(
        gold_ids,
        doc_category_keywords_map,
        max_keys=GOLD_SELECTED_MAX_KEYS,
        fallback=default_gold_selected_categories,
    )
    sample = Round3Sample(
    ...
    gold_selected_categories_by_sample.append(selected_gold_categories)
...
for iter_idx in range(hp.NUM_ITERS):
    ...
    # Intent: iter0 uses the same rewrite prompt as later iterations for clean gold-key analysis.
    ...
    for sample_idx, sample in enumerate(tqdm(
        ...
        if rewrite_enabled and do_rewrite:
            ...
            rewrite_candidates.append({
                ...
                "gold_selected_categories": gold_selected_categories_by_sample[sample_idx],
    ...
    if rewrite_candidates:
        ...
        for meta in rewrite_candidates:
            ...
            gold_selected_categories = [str(x).strip() for x in (meta.get("gold_selected_categories") or []) if str(x).strip()]
            ...
            prompt = _append_selected_corpus_category_keys_prompt(prompt, gold_selected_categories)
            ...
            if (not hp.REWRITE_FORCE_REFRESH) and (cache_key in rewrite_map):
                ...
                sample.rewrite_history.append({
                    ...
                    "gold_selected_categories": gold_selected_categories,
        ...
        if rewrite_prompts:
            ...
            for meta, out in zip(rewrite_meta, rewrite_outputs):
                ...
                sample.rewrite_history.append({
                    ...
                    "gold_selected_categories": meta.get("gold_selected_categories", []),
                    "action": sample.last_action,
                    "actions": sample.last_actions,
                    "possible_docs": sample.last_possible_docs,
                    "rewrite": rewrite,
