...
- 코드를 어떻게 구현할지 / 파일을 어떻게 바꿀지 설명을 먼저 하고 그 다음에 구현해.
...
- LLM이 주어진 정보가 쿼리에 대한 답으로 충분한지, 충분하지 않은지 internal knowledge로 알 수 있다면, retrieval 자체가 필요 없는 상황 아니냐?
	...
	- 하지만 지식을 abstraction -> abstraction 카테고리별로 지식이 나눠져 있다면, 카테고리별로 yes/no 판단하는 능력은 모델이 갖고 있다고 전제할 수 있음.
		- = 카테고리 레벨로 navigate하는건 llm이 모든 세부적인 지식 없이 할 수 있지 않을까 / 또는 tail-data라면 학습할 수 있지 않을까

## .md 파일을 작성할 때
- 쓸데없이 말을 길게 하지 마.
- ~가 아니라 ~이다. 라는 표현을 삼가주세요. 나중에 볼 때 무슨 소린지 모르겠어요
- 어떤 목적을 가지고 있는지, 구현의 핵심 알고리즘, 기존 구현과의 차별점, analysis의 경우 실행 방법과 핵심 결과를 적어야 함