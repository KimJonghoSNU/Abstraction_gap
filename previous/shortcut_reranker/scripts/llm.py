...
class Base_LLM:
    def __init__(self, model_name_or_path, temperature=0.0, top_p=1.0, top_k=1, max_model_len=32768, 
        ...
        # self.dtype = self.kwargs.get("llm_dtype", "float32")
        self.dtype = self.kwargs.get("llm_dtype", "auto")
        ...
        if self.snowflake:
            ...
            if os.path.exists('/data/models/Llama-3.3-70B-Instruct'):
                tokenizer_path = '/data/models/Llama-3.3-70B-Instruct'
            else:
                tokenizer_path = '/data4/jaeyoung/models/Llama-3.3-70B-Instruct'
    ...
    def llm_batch_inference(self, batch_prompts, stop=None, max_tokens=None, logprobs=None, return_n_tokens=False, num_generation=1) -> List[str]:
        if self.snowflake:
            ...
            if num_generation >= 2:
                ...
                output_texts = [[text.text for text in output.outputs] for output in output_list]
            else:
                output_list = self.model.generate(
                    batch_prompts, 
                    lora_request=self.lora_request, 
