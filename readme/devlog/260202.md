...
### 2026-02-04 업데이트: "검색 결과가 depth-1 한 브랜치에 몰려있는가?" 점검

- 구현 업데이트
    - `scripts/eval_round3_graph_prompt_decomp.py`에 depth-1 집중도 지표를 추가
        - `MeanUniqueDepth1Branches@10_{pre,post}`: top-10이 덮는 depth-1 브랜치 개수 평균
        - `SingleDepth1BranchRate@10_{pre,post}`: top-10이 단일 depth-1 브랜치에만 있는 샘플 비율
        - `MeanMaxDepth1Share@10_{pre,post}`: top-10에서 가장 큰 depth-1 브랜치 점유율 평균
        - 각 지표의 delta와 DoD(`DeltaPrompt_Delta_*`)도 함께 저장

- 현재 저장된 결과(`all_eval_sample_dicts.pkl`)로 직접 확인한 요약 (5개 subset 평균, iter0)
    - `MeanUniqueDepth1Branches@10_pre` = **2.19**
    - `SingleDepth1BranchRate@10_pre` = **26.71%**
    - `MeanMaxDepth1Share@10_pre` = **77.18%**
    - 해석: 완전히 한 브랜치만 보는 건 아니지만, top-10은 실질적으로 상위 1개 depth-1 브랜치에 강하게 집중됨

- iter0 -> iter1 변화 (ours, 5개 subset 평균)
    - `Delta_MeanUniqueDepth1Branches@10` = **-0.182**
    - `Delta_SingleDepth1BranchRate@10` = **+7.03pp**
    - `Delta_MeanMaxDepth1Share@10` = **+2.56pp**
    - 해석: 첫 rewrite 후에는 분산보다 집중이 더 커지는 경향이 있음

- DoD (ours - baseline, 변화량 기준)
    - `DeltaPrompt_Delta_MeanUniqueDepth1Branches@10` = **-0.058**
    - `DeltaPrompt_Delta_SingleDepth1BranchRate@10` = **+3.29pp**
    - `DeltaPrompt_Delta_MeanMaxDepth1Share@10` = **+1.17pp**
    - 해석: action_v1이 thinkqe 대비 top-10의 depth-1 집중을 더 줄였다는 근거는 현재 약함(오히려 약간 더 집중)

### 2026-02-04 업데이트: "Parent는 맞는데 Leaf를 못 맞히는가?" (D4 -> D5 진단)

- 먼저 확인: `region hit` 계산 정의
    - 맞음. `scripts/eval_round3_graph_prompt_decomp.py`에서 `top-10 retrieved leaf`와 `gold leaf`를 path prefix로 비교해 계산함
    - `RALR=none`인 현재 실험에서는 `anchor_leaf_paths`(leaf-only 순서)가 평가 입력임
    - 즉 `RegionHitD4@10`은 "gold와 같은 depth-4 parent prefix를 맞췄는가", `RegionHitD5@10`은 더 세부 depth-5까지 맞췄는가를 의미

- 함께 본 지표 (ours, 5개 subset 평균, iter1=post)
    - `RegionHitD4@10_post` = **67.45**
    - `RegionHitD5@10_post` = **65.87**
    - gap(`D4-D5`) = **1.58pp**
    - `P_RegionHitD5@10_given_D4_post` = **97.30%**
    - `D4_Hit_D5_MissRate@10_post` = **1.58%**

- 해석 / 결론
    - 결론적으로, **"같은 parent 안에서만 reranking"으로 얻을 수 있는 Hit@10 상승 headroom은 작다** (약 **+1.6pp** 상한)
    - 즉 현재 병목은 "parent를 맞췄는데 sibling을 못 고르는 문제"도 있지만, 그 크기는 제한적
    - 더 큰 병목은 여전히 parent region 진입 자체(깊은 depth로 갈수록 hit 하락)와 candidate generation 측면일 가능성이 큼
    - 따라서 same-parent rerank는 **주로 nDCG 정렬 개선용**으로 보고, 큰 recall/hit 상승을 기대하긴 어려움

- 참고: AncestorHit vs AnchorLeafHit (iter1, 5 subset 평균)
    - `AncestorHit@10` = **72.45%** (branch-level prefix hit)
    - `AnchorLeafHit@10` = **65.87%** (leaf exact hit)
    - branch 수준 신호는 더 많이 잡지만, leaf exact match로의 변환에서 일부 손실이 남아 있음
