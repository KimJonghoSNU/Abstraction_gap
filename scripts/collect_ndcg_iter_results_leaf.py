...
def _format_flat_overall_iter_results(df: pd.DataFrame, metric: str) -> pd.DataFrame:
    if df.empty:
        return df
    # Intent: match the summary CSV interpretation by averaging subset means, not pooled queries.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Collect per-iteration mean metric summaries from leaf_iter_metrics.jsonl files."
    )
    parser.add_argument("--base_dir", type=str, default="results/BRIGHT", help="Base results directory")
    parser.add_argument(
        "--out_csv",
        type=str,
        default="results/BRIGHT/ndcg_iter_summary_leaf.csv",
